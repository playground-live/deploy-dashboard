...
| フィールド | 役割 | 更新元 |
|---|---|---|
| `repositoryId` | GitHub リポジトリの不変 ID（識別子） | CD から自動取得（`github.repository_id`） |
| `name` | ダッシュボード上の表示名 | フロントエンドから編集可能。初回は repositoryName がデフォルト |
| `repositoryName` | GitHub リポジトリ名 | CD から毎回自動更新（リネームに追従） |
...
`repositoryId` と `repositoryName` は `github.repository_id` / `github.event.repository.name` から自動取得されるため、指定不要。サービスが未登録の場合はリポジトリ名をデフォルトの表示名として自動登録される。
