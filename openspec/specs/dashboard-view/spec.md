...
Deploy Dashboard のトップページ (`/`) で、社内開発者が「各環境 × 全サービスのデプロイ最新版」を 1 画面で把握するための中核 UI。サービスごとのデプロイ履歴閲覧、`ServiceGroup` による行のグルーピング、30 秒間隔の自動リフレッシュと、裏側の 2 つの GET API (`/api/deployments`, `/api/deployments/history`) の挙動契約を定義する。
